 ...
 */
export function itensVisiveis(model: NavModel, filtro?: FiltroDeItem): NavItem[] {
  return filtro ? model.items.filter(filtro) : [...model.items];
}
 ...
 */
export function itensDaBarraInferior(model: NavModel, filtro?: FiltroDeItem): NavItem[] {
  return itensVisiveis(model, filtro)
    .filter((i) => i.bottom)
 ...
 */
export function gruposComItens(
  model: NavModel,
  filtro?: FiltroDeItem,
): { grupo: NavGroupDef; itens: NavItem[] }[] {
  const visiveis = itensVisiveis(model, filtro);
  return model.groups
    .map((grupo) => ({ grupo, itens: visiveis.filter((i) => i.group === grupo.id) }))
